  ...
  async check() {
    ...
    for (const token of tokens) {
      ...
      if (token.faucetPrivateKey) {
        ...
        const faucetParsedAccountInfo =
          await this.connection.getParsedAccountInfo(faucet.publicKey);
        const faucetArray = [
          ...(faucetParsedAccountInfo.value?.data as Buffer),
        ];
        assert(
          new PublicKey(token.mint).equals(
            new PublicKey(faucetArray.slice(45, 77)),
          ),
        );
        ...
        const faucetParsedAccountInfo =
          await this.connection.getParsedAccountInfo(faucet);
        const faucetArray = [
          ...(faucetParsedAccountInfo.value?.data as Buffer),
        ];
        assert(
      ...
      if (token.mintPrivateKey) {
        ...
        assert(
          mint.publicKey.toBase58() == new PublicKey(token.mint).toBase58(),
        );
        console.log(
          `  mint = ${JSON.stringify(
            await this.connection.getParsedAccountInfo(mint.publicKey),
          )}`,
        );
        console.log(
          `  supply: ${await this.getMintSupply(
        ...
        console.log(
          `  mint = ${JSON.stringify(
            await this.connection.getParsedAccountInfo(mint),
          )}`,
        );
        console.log(
          `  supply: ${await this.getMintSupply(
          ...
          if (!accountInfo) {
            ...
            transaction.add(
              SystemProgram.createAccount({
                ...
                space: 82,
                lamports:
                  await this.connection.getMinimumBalanceForRentExemption(82),
          ...
          if (!accountInfo) {
            ...
            transaction.add(
              SystemProgram.createAccount({
                ...
                space: 77,
                lamports:
                  await this.connection.getMinimumBalanceForRentExemption(77),
              }),
              createSetAuthorityInstruction(
                mint,
              ...
              new TransactionInstruction({
                ...
                data: Buffer.from([
                  0,
                  ...faucetLimit
                    .mul(new BN(pow10(token.decimals)))
                    .toArray('le', 8),
                ]),
                keys: [
