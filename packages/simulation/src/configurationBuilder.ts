...
export default class ConfigurationBuilder {
  ...
  public async build() {
    const generatedTokenConfigurations = this.instruments.tokens.map(
      (symbol: string) => {
        const tokenConfiguration = SOLANA_TOKENS.find(tokenConfiguration => {
          return symbol === tokenConfiguration.symbol;
        });
        ...
        switch (symbol) {
          case 'MSRM': {
            return {
              ...
              faucetPrivateKey:
                'SxVwfF+aJHsk4Mj1cr0Laooc1SMQQx4Z2EgnY7Dw1NhJlPv9Rv70+wKpORJYoMruAbmVrEuLN+8/ggZKTyt/7g==',
              mint: 'HiihFm4YSoiMLnp4Y35riBu9ieDj9LzTA3NwbDEe9mc3',
              mintPrivateKey:
                '76iyRD9P+j3t7xvtmFAnvvYunwckZSZypDwnyoOAdPj4avKVygrNevUazPBJjfl5egwZek2202iFgyrBwp3v0A==',
            };
          }
          case 'SOL': {
            return {
          ...
          case 'SRM': {
            return {
              ...
              faucetLimit: 1_000_000_000,
              faucetPrivateKey:
                'FvIcq+ciTasVSFL9Wa6qb7r8iOiF6ov0HtRBL6qqV94bBRY43fYoX2ZMhT9uQbdaU0UYD2/d8DIRbb003ZDztQ==',
              mint: '5zq1tdJfRZ8boTGjKE9dCA77nyFEm4c1uji8Q3nfWfaY',
              mintPrivateKey:
                'XE5yUmDjNCFyLVUw0Ke00OO6kLy2I8gLqByZ4HQ2FjZKPyUbUqNG4WR3CEE6ibHjOGAbxEtECZ/gZVlU9m+l+Q==',
            };
          }
          default: {
            if (
              this.config.devnet.tokens &&
              this.config.devnet.tokens[symbol]
            ) {
              return {
                symbol,
                name: tokenConfiguration.name,
                ...
                faucetPrivateKey:
                  this.config.devnet.tokens[symbol].faucetPrivateKey,
                mint: this.config.devnet.tokens[symbol].mint,
                mintPrivateKey:
                  this.config.devnet.tokens[symbol].mintPrivateKey,
              };
            } else {
              ...
              return {
                ...
                faucetLimit: 1_000_000_000,
                faucetPrivateKey: Buffer.from(faucetKeypair.secretKey).toString(
                  'base64',
                ),
                mint: mintKeypair.publicKey.toBase58(),
                mintPrivateKey: Buffer.from(mintKeypair.secretKey).toString(
                  'base64',
                ),
              };
            }
    ...
    if (this.instruments.oracles && this.instruments.oracles.length > 0) {
      ...
      generatedOracleConfigurations = await Promise.all(
        this.instruments.oracles.map(async (symbol: string) => {
          const oracleConfiguration = PYTH_ORACLES.find(oracleConfiguration => {
            return symbol === oracleConfiguration.symbol;
          });
          assert(oracleConfiguration);
          if (
            this.config.devnet.oracles &&
            this.config.devnet.oracles[symbol.replace('/', '_')]
          ) {
            return this.config.devnet.oracles[symbol.replace('/', '_')];
          } else {
            const oracleKeypair = Keypair.generate();
            const productKeypair = Keypair.generate();
            const pythPrice = await pythClient.getPythPrice(
              new PublicKey(oracleConfiguration.address),
            );
            return {
              symbol,
              address: oracleKeypair.publicKey,
              privateKey: Buffer.from(oracleKeypair.secretKey).toString(
                'base64',
              ),
              product: productKeypair.publicKey,
              productPrivateKey: Buffer.from(productKeypair.secretKey).toString(
                'base64',
              ),
    ...
    if (this.instruments.markets.length > 0) {
      generatedMarketConfigurations = await Promise.all(
        this.instruments.markets.map(async (symbol: string) => {
          const marketConfiguration = SERUM_MARKETS.find(
            marketConfiguration => {
              return symbol === marketConfiguration.symbol;
            },
          );
          assert(marketConfiguration);

          const baseToken = generatedTokenConfigurations.find((token: any) => {
            return marketConfiguration.baseSymbol === token.symbol;
          });
          const quoteToken = generatedTokenConfigurations.find((token: any) => {
            return marketConfiguration.quoteSymbol === token.symbol;
          });

          if (
            this.config.devnet.markets &&
            this.config.devnet.markets[symbol.replace('/', '_')]
          ) {
            assert(
              (this.config.devnet.markets[symbol.replace('/', '_')].baseMint =
                baseToken.mint),
            );
            assert(
              (this.config.devnet.markets[
            ...
            assert(
              (this.config.devnet.markets[symbol.replace('/', '_')].quoteMint =
                quoteToken.mint),
            );
            assert(
              (this.config.devnet.markets[
            ...
            return {
              ...
              baseVaultPrivateKey: Buffer.from(
                baseVaultKeypair.secretKey,
              ).toString('base64'),
              ...
              bidsPrivateKey: Buffer.from(bidsKeypair.secretKey).toString(
                'base64',
              ),
              asks: asksKeypair.publicKey.toBase58(),
              asksPrivateKey: Buffer.from(asksKeypair.secretKey).toString(
                'base64',
              ),
              quoteDustThreshold: new BN(
                marketConfiguration.quoteDustThreshold,
              ).toNumber(),
    ...
    if (this.instruments.tokens.length > 0) {
      generatedPoolConfigurations = await Promise.all(
        this.instruments.tokens
          .map(async (symbol: string) => {
            const token = generatedTokenConfigurations.find((token: any) => {
              return token.symbol == symbol;
            });
            const oracle = generatedOracleConfigurations.find(oracle => {
              return oracle.symbol == symbol + '/USD';
            });
            if (
              this.config.devnet.pools &&
              this.config.devnet.pools[symbol.replace('/', '_')]
            ) {
              ...
              return {
                ...
                feesVaultPrivateKeyPrivateKey:
                  pool.feesVaultPrivateKeyPrivateKey,
              };
            } else {
              if (oracle) {
                ...
                return {
                  ...
                  feesVaultPrivateKeyPrivateKey: Buffer.from(
                    feesVaultKeypair.secretKey,
                  ).toString('base64'),
                };
              }
            }
    ...
    if (this.instruments.swaps.length > 0) {
      generatedSwapConfigurations = await Promise.all(
        this.instruments.swaps
          .map(async (symbol: string) => {
            if (
              this.config.devnet.swaps &&
              this.config.devnet.swaps[symbol.replace('/', '_')]
            ) {
              return this.config.devnet.swaps[symbol.replace('/', '_')];
            } else {
              const swapConfiguration = TOKEN_SWAPS.find(swapConfiguration => {
                return symbol === swapConfiguration.symbol;
              });
              if (swapConfiguration) {
                const baseToken = generatedTokenConfigurations.find(
                  (token: any) => {
                    return swapConfiguration.baseSymbol === token.symbol;
                  },
                );
                assert(baseToken);
                const quoteToken = generatedTokenConfigurations.find(
                  (token: any) => {
                    return swapConfiguration.quoteSymbol === token.symbol;
                  },
                );
                assert(quoteToken);
                ...
                const [authority, bumpSeed] =
                  await PublicKey.findProgramAddress(
                    [swapKeypair.publicKey.toBuffer()],
                    new PublicKey(PROGRAMS.devnet.orca_token_swap),
                  );
                ...
                const recipientAccountOwnerKeypair: Keypair =
                  Keypair.generate();

                const feeAccount = await getAssociatedTokenAddress(
                  poolTokenMintKeypair.publicKey,
                  feeAccountOwnerKeypair.publicKey,
                );
                const recipientAccount = await getAssociatedTokenAddress(
                  poolTokenMintKeypair.publicKey,
                  recipientAccountOwnerKeypair.publicKey,
                );

                return {
                  symbol,
                  swap: swapKeypair.publicKey.toBase58(),
                  swapPrivateKey: Buffer.from(swapKeypair.secretKey).toString(
                    'base64',
                  ),
                  authority: authority,
                  poolTokenMint: poolTokenMintKeypair.publicKey.toBase58(),
                  poolTokenMintPrivateKey: Buffer.from(
                    poolTokenMintKeypair.secretKey,
                  ).toString('base64'),
                  ...
                  feeAccountOwnerPrivateKey: Buffer.from(
                    feeAccountOwnerKeypair.secretKey,
                  ).toString('base64'),

                  recipientAccount: recipientAccount,
                  recipientAccountOwner:
                    recipientAccountOwnerKeypair.publicKey.toBase58(),
                  recipientAccountOwnerPrivateKey: Buffer.from(
                    recipientAccountOwnerKeypair.secretKey,
                  ).toString('base64'),

                  tradeFeeNumerator: swapConfiguration.tradeFeeNumerator,
                  tradeFeeDenominator: swapConfiguration.tradeFeeDenominator,
                  ownerTradeFeeNumerator:
                    swapConfiguration.ownerTradeFeeNumerator,
                  ownerTradeFeeDenominator:
                    swapConfiguration.ownerTradeFeeDenominator,
                  ownerWithdrawFeeNumerator:
                    swapConfiguration.ownerWithdrawFeeNumerator,
                  ownerWithdrawFeeDenominator:
                    swapConfiguration.ownerWithdrawFeeDenominator,
    ...
    return [
      {
        devnet: await this.buildPublic(
          'devnet',
          PROGRAMS.devnet,
          generatedMarketConfigurations,
          generatedOracleConfigurations,
          generatedPoolConfigurations,
          generatedSwapConfigurations,
          generatedTokenConfigurations,
        ),
        localnet: await this.buildPublic(
          'localnet',
          PROGRAMS.localnet,
          generatedMarketConfigurations,
          generatedOracleConfigurations,
        ...
        'mainnet-beta': await this.buildPublic(
          'mainnet',
          PROGRAMS.mainnet,
          SERUM_MARKETS,
          PYTH_ORACLES,
      ...
      {
        devnet: await this.buildPrivate(
          'devnet',
          PROGRAMS.devnet,
          generatedMarketConfigurations,
          generatedOracleConfigurations,
        ...
        localnet: await this.buildPrivate(
          'localnet',
          PROGRAMS.localnet,
          generatedMarketConfigurations,
          generatedOracleConfigurations,
  ...
  private async buildPrivate(
    ...
    for (const symbol of this.instruments.markets) {
      const marketConfiguration = marketConfigurations.find(
    ...
    for (const symbol of this.instruments.oracles) {
      const oracleConfiguration = oracleConfigurations.find(
        (oracleConfiguration: any) => {
          return oracleConfiguration.symbol === symbol;
        },
      );
      assert(oracleConfiguration);
    ...
    for (const symbol of this.instruments.tokens) {
      const poolConfiguration = poolConfigurations.find(
        (poolConfiguration: any) => {
          return poolConfiguration && symbol === poolConfiguration.symbol;
        },
      );
    ...
    for (const symbol of this.instruments.swaps) {
      const swapConfiguration = swapConfigurations.find(
        (swapConfiguration: any) => {
          return swapConfiguration && symbol === swapConfiguration.symbol;
        },
      );
      if (swapConfiguration) {
    ...
    const tokens: any = {};
    for (const symbol of this.instruments.tokens) {
      const tokenConfiguration = tokenConfigurations.find(
        (tokenConfiguration: any) => {
          return symbol === tokenConfiguration.symbol;
        },
      );
      assert(tokenConfiguration);
  ...
  private async buildPublic(
    ...
    let configuration = await this.buildPrivate(
      environment,
      marginAccounts,
      marketConfigurations,
      oracleConfigurations,
      poolConfigurations,
      swapConfigurations,
      tokenConfigurations,
    );
  ...
  private filterSecretKeys(privateObj: any) {
    ...
    for (const key of Object.keys(privateObj)) {
      const privateItem = privateObj[key];
      const publicItem: any = {};
      for (const key2 of Object.keys(privateItem)) {
        if (
          !key2.endsWith('rivateKey') &&
          !key2.endsWith('price') &&
          !key2.endsWith('confidence')
        ) {
