...
async function monitor() {
  const accounts = [
    new Account(
      JSON.parse(
        fs.readFileSync(os.homedir() + `/.config/solana/maker.json`, 'utf-8'),
      ),
    ),
    new Account(
      JSON.parse(
        fs.readFileSync(os.homedir() + `/.config/solana/taker.json`, 'utf-8'),
      ),
    ),
  ];
  ...
  const oracles = Object.keys(mainnetConfig.oracles).map(key => {
    return mainnetConfig.oracles[key];
  });

  for (const oracle of oracles) {
    console.log(`ORACLE: ${oracle.symbol}`);
    const oracleAccount = await mainnetConnection.getAccountInfo(
  ...
  for (const market of markets) {
    ...
    if (marketAccount) {
      ...
      console.log(
        `  referrerRebatesAccrued = ${marketState.referrerRebatesAccrued}`,
      );
      ...
      for (const account of accounts) {
        ...
        console.log(
          `  openOrdersAccounts = ${JSON.stringify(openOrdersAccounts)}`,
        );
        ...
        for (const openOrdersAccount of openOrdersAccounts) {
          ...
          console.log(
            `    openOrders.referrerRebatesAccrued = ${Number(
